...
int[] Time = {4};//1,
int[] LifeTime = {2,3,4,5,6,7,8,9,10};//

for (int Lloop=0 ; Lloop<LifeTime.length ; Lloop++)
 {
...
/*
 * official topology: beta=0.9, alpha=0.7, prob=0.5, gives links/node ~6-7, and edges~350-380.
 * to test impact of topologies on processing time, delay, AR, PC and PS, the following tests are needed:
 * test-1 beta=0.9, alpha=0.7, prob=0.5  links/node ~6-7, and edges~350-380
 * test-2 beta=0.9, alpha=0.8, prob=0.5, links/node = 10, edges=530
 * test-3 beta=0.9, alpha=0.9, prob=0.5  links/node = 12, edges=630
 * test-4 beta=0.9, alpha=1.0, prob=0.5  links/node = 15, edges=770
 * test-4 beta=1.0, alpha=1.0, prob=0.5  links/node = 17, edges=860
 */
...
int Minsd                = 1;  //1 minimum delay in a SN path.
int Maxsd                = 50;	 //50 Maximum delay in a SN path.
...
SN_Nodes_MAX_Power_Cap   [i] =  Min_PWCONS + 15 * SN_Nodes_MAX_CPU_Cap[i];  //(double) Math.round((Min_SN_DCPw   + (double)(Math.random() * ((Max_SN_DCPw     -   Min_SN_DCPw)        + 1)))); // fill the SN node's power with random values between 0-20.
...
for ( int Tot_Num_of_VNRs = 0; Tot_Num_of_VNRs < MaximumVNRs; Tot_Num_of_VNRs++ )
{
	...
	{    					
			...
			int NodesinVNRpath = LifeTime[Lloop];//(Minnodespervnr + (int)(Math.random() * ((Maxnodespervnr  - Minnodespervnr)  + 1)));
			...
			int Maxvd	       = 100;  // maximum delay in msec.
			int Minvd	       = 20;  // minimum delay in msec.
...
System.out.println("Run"+Runs+"\t"+"Load\t"+"\t"+"Time"+Time[Tloop]+"\t"+"VNR#\t" + VNR);
...
int L = 5*Time[Tloop];// (int) (2 + Math.random() * ((10 - 2)  + 1));
int comb=(int)(2 + Math.random() * ((40 - 2)  + 1));
if (VNR >= L )
{ System.out.println("Online Check");
	...
	{ System.out.println("Online Started to expire VNRs");
		...
		{ 
			//int Rem = VNR-L+i;
			System.out.println("Run"+Runs+"\t"+"Load\t"+"\t"+"Time"+Time[Tloop]+"\t"+"VNR#\t" + VNR +"\t"+ "Rem" + Rem + " SN_Selected_Path[Rem][0]\t"+SN_Selected_Path[Rem][0]);
...
//-----------------------------------------------------------------------------------------------------------------------	
	...
	double Embedding_VNRcost	= Embedding_cost[VNR]/(5*Time[Tloop]); // this is where we calculate the embedding cost. Alfa and Beta are defined as equal to 0.5.
	...
	double PC = (Tot_SN_consumed_power - 165*Turned_off_nodes-165*(SN_Nodes-Turned_off_nodes))/(20*Maxnodespervnr/2);//(*(SN_Nodes-Turned_off_nodes));
	...
	VNRs_output[VNR][11] = LifeTime[Lloop];
	VNRs_output[VNR][12] = Time[Tloop];
	VNRs_output[VNR][13] = SN_P2_nodes.length;//Embedding_VNRcost
	VNRs_output[VNR][14] = edgesnum/SN_Nodes;//Revnew
   ...
   (int)VNRs_output[VNR][14] +"\t Rev\t");
	...
	try {	
		BufferedWriter writerVNROUT =						
				new BufferedWriter ( new FileWriter("D-2-10-VNRs-ZhongVNRsDetailed.txt", true));
...
try {	
BufferedWriter writerVNROUT =						
	new BufferedWriter ( new FileWriter("D-2-10-VNRs-ZhongVNRs.txt", true));
...
try {	
BufferedWriter writerVNROUT =						
	new BufferedWriter ( new FileWriter("D-2-10-VNRs-ZhongRunsFinal.txt", true));
